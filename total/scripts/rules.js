...
document.getElementById("set_d61").checked = true;
...
function save_data() {
    ...
    if (document.getElementById("set_d61").checked) {
        round = !player_color;
    }
    else {
        round = player_color;
    ...
    if (difficulty && (player_color == round)) {
        if (!field) {
            setTimeout("popup('0', true)", 6000);
        }
        else {
            setTimeout("popup('0', true)", 5200);
        }
    }
}
...
// Choise centeral square achate
function square_popup(e, acces) {
    unfilled = [];
    for (i = ((field_data[0].value) / 2) - 1; i < ((field_data[0].value) / 2) + 1; i++) {
        for (j = ((field_data[0].value) / 2) - 1; j < ((field_data[0].value) / 2) + 1; j++) {
            if (!items_info[i].content[j].type) {
                unfilled.unshift(((i * 10) + j));
            }
        }
    }
    if (difficulty && (player_color == round)) {
        random_choose = Math.floor(Math.random() * unfilled.length);
        ten = Math.floor(parseInt(unfilled[random_choose]) / 10);
        one = parseInt(unfilled[random_choose]) - (ten * 10);
        if (player_color) {
            document.getElementById(unfilled[random_choose]).className = 'item white';
            items_info[ten].content[one].type = 'w';
            document.getElementById("turner").className = 'item black';
            round = false;
        }
        else {
            document.getElementById(unfilled[random_choose]).className = 'item black';
    ...
    if (fill_square && difficulty && (player_color == round)) {
        setTimeout("popup('0', true)", 700);
    }
    else if (!fill_square && difficulty && (player_color == round)) {
        setTimeout("valence()", 700);
